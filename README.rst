...
wpm will save scores in a CSV file in `~/.local/share/wpm/wpm.csv`. This file can be loaded
...
the existing file into `~/.local/share/wpm/wpm.csv.backup` and create a new one.
...
The ~/.config/wpmrc file
-----------------

The first time you start wpm, it writes a `wpmrc` file to `~/.config/`.
